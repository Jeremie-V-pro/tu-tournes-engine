...
namespace lve
{
  ...
  FirstApp::FirstApp()
  {
    ...
    // Load computed texture
    texturesrc = std::make_unique<LveTexture>(lveDevice, "textures/pp.png", true);
    texturedst = std::make_unique<LveTexture>(lveDevice, "textures/dd.png", true);

    // Create pool for computed texture
    ...
    // Create descriptor set layout for computed texture
    LveDescriptorSetLayout::Builder setLayoutcpBuilder(lveDevice);
    setLayoutcpBuilder.addBinding(0, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, VK_SHADER_STAGE_COMPUTE_BIT);
    setLayoutcpBuilder.addBinding(1, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, VK_SHADER_STAGE_COMPUTE_BIT);
    testlayout = setLayoutcpBuilder.build();

    VkDescriptorImageInfo imageDescriptorInfosrc{};
    imageDescriptorInfosrc.sampler = texturesrc->getSampler();
    imageDescriptorInfosrc.imageView = texturesrc->getImageView();
    imageDescriptorInfosrc.imageLayout = texturesrc->getImageLayout();

    VkDescriptorImageInfo imageDescriptorInfodst{};
    imageDescriptorInfodst.sampler = texturedst->getSampler();
    imageDescriptorInfodst.imageView = texturedst->getImageView();
    imageDescriptorInfodst.imageLayout = texturedst->getImageLayout();

    LveDescriptorWriter(*testlayout, *texturePool)
        .writeImage(0, &imageDescriptorInfosrc)
        .writeImage(0, &imageDescriptorInfodst)
  ...
  void FirstApp::run()
  {
    ...
    SimpleComputeSystem simpleComputeSystem{lveDevice,
                                            testlayout->getDescriptorSetLayout()};
    LveCamera camera{};
