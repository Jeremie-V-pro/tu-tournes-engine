#include "simple_compute_systemG.hpp"
...
namespace lve
{
  ...
  SimpleComputeSystemG::SimpleComputeSystemG(LveDevice &device, VkDescriptorSetLayout textureSetLayout) : lveDevice{device}
  {
    createPipelineLayout(textureSetLayout);
    createPipeLine();
  }
  SimpleComputeSystemG::~SimpleComputeSystemG()
  {
    vkDestroyPipelineLayout(lveDevice.device(), pipelineLayout, nullptr);
  }

  void SimpleComputeSystemG::createPipelineLayout(VkDescriptorSetLayout textureSetLayout)
  {
  ...
  void SimpleComputeSystemG::createPipeLine()
  {
    ...
    lveCPipeline = std::make_unique<LveCPipeline>(
        lveDevice, "shaders/simple_compute_shaderG.comp.spv", pipelineConfig);
  }

  void SimpleComputeSystemG::executeCpS(FrameInfo frameInfo, VkDescriptorSet computeDescriptorSets, VkExtent2D windowExtent)
  {
