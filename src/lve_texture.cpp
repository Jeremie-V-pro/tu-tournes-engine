...
namespace lve
{
    ...
    LveTexture::LveTexture(LveDevice &device, const std::string &filepath, LveDescriptorSetLayout *textureSetLayout) : lveDevice{device}
    {
        int width, height, channels;
        int byPerPixel;

        std::cout << std::filesystem::exists((ENGINE_DIR + filepath).c_str()) << std::endl;
        ...
        imageDescriptorInfo.sampler = this->getSampler();
        imageDescriptorInfo.imageView = this->getImageView();
        imageDescriptorInfo.imageLayout = this->getImageLayout();
