...
layout(set = 2, binding = 0) uniform sampler2D displacement;
...
void main() {
    ...
    diffuseLight += cosAngIncidence;
    // Specular Sun
    vec3 halfAngleSun = normalize(sunDirectionNorm + viewDirection);
    float blinnTermSun = dot(surfaceNormal, halfAngleSun);
    blinnTermSun = clamp(blinnTermSun, 0, 1);
    blinnTermSun = pow(blinnTermSun, 65536.0);
